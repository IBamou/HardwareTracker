...
function getEmployeesByCategory($categoryId) {
    try {
        $sql = 'SELECT * FROM employees WHERE category_id = :id';
        $stmt = $GLOBALS['db']->prepare($sql);
...
function updateEmployeeStatus($employee_id, $status, $category_id) {
    try {
        $sql = "UPDATE employees SET category_id = :category_id, status = :status WHERE id = :employee_id";
        $stmt = $GLOBALS['db']->prepare($sql); 
        $stmt-> execute([':category_id' => $category_id, ':status' => $status,':employee_id' => $employee_id]);
...
function returnAssignment($employee_id) {
    try {
        $sql = 'UPDATE assignments SET returned_at = NOW() WHERE employee_id = :employee_id AND returned_at IS NULL';
        $stmt = $GLOBALS['db']->prepare($sql); 
        $stmt-> execute([':employee_id'=> $employee_id]);
    } catch (Exception $e) {
        echo ''. $e->getMessage();
